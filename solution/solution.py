...
def run_assistant(
    ...
    while True:
        ...
        stream = retry_with_backoff(_call_stream)
        ...
        u_tokens = count_tokens(user_msg, model=OPENAI_MODEL)
        r_tokens = count_tokens(reply, model=OPENAI_MODEL)
        tokens_used += u_tokens + r_tokens
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for result in results:
        g_resp = result.get("gpt4o_response") or result.get("gpt4o_answer", "")
        m_resp = result.get("mini_response") or result.get("mini_answer", "")
        g_lat = result.get("gpt4o_latency") if result.get("gpt4o_latency") is not None else result.get("gpt4o_time", 0.0)
        m_lat = result.get("mini_latency") if result.get("mini_latency") is not None else result.get("mini_time", 0.0)
